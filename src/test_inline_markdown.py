...
class TestTextToTextNodes(unittest.TestCase):
    ...
    def test_bold_split(self):
        node = "This is **BOLD** **text** with an _italic_ word and a `code block` and an ![obi wan image](https://i.imgur.com/fJRm4Vk.jpeg) and a [link](https://boot.dev)"
        new_node = text_to_textnodes(node)
        self.assertEqual(
            new_node,
            [
    ...
    def test_no_split(self):
        node = "This text contains nothing to be split, so it should only return a TextNode with a TextType.TEXT"
        new_node = text_to_textnodes(node)
        self.assertEqual(
            new_node,
            [
                TextNode("This text contains nothing to be split, so it should only return a TextNode with a TextType.TEXT", TextType.TEXT),
            ]
        )